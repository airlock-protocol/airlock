...
- ``GET /passport/{did}/status`` — registration, revocation and
  reputation summary for one agent DID.

Both return 404 while ``passport_enabled`` is off.
...
private keys — so the directory is served unsigned. Verifiers that insist
on directory response signatures would need agents to self-host their
directories instead.
"""
...
from airlock.schemas.passport import PassportStatus, ReputationSummary
...
async def signatures_directory(request: Request) -> Response:
    ...
    for did in sorted(registry):
        ...
        if await revocation_store.is_revoked(did):
            continue
        try:
            keys.append(resolve_public_key(did))
        except ValueError:
            logger.debug("Skipping non-Ed25519 DID in directory: %s", did)
            continue
